...
export async function performLogin(){
    ...
    if(!clerkFrontendApi || !clientId ){
        throw new Error("CLERk ENV missing")
    }
        ...
        const server = Bun.serve({
            ...
            async fetch(req){
                ...
                if(error){
                    ...
                    return new Response(`Authentication failed :${msg}`,{status:404})
                }
                ...
                try {
                    ...
                    const message =getErrorMessage(error)
                     setTimeout(()=>server.stop(),500)
                    return new Response(`Authentication stfailedate: ${message}`,{status:400})
                }
            }
        })
